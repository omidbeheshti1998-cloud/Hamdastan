  ...
  return (
    <div>
      ...
      <div key={question.id} className="animate-step-in">
        <h1 className="mb-6 text-xl leading-9 font-bold tracking-tight text-zinc-900 dark:text-zinc-50">
          {question.question}
        </h1>
          ...
          {question.choices.map((choice) => {
            ...
            return (
              <button
                ...
                className={`min-h-18 w-full rounded-xl border px-5 py-4 text-start text-base leading-8 transition ${
                  selected
      ...
      <div className="mt-6 flex items-center justify-between">
        <button
          type="button"
          onClick={goBack}
          className="h-11 px-2 text-sm font-medium text-zinc-500 transition hover:text-zinc-900 dark:hover:text-zinc-100"
        >
          قبلی
        </button>
        <button
          type="button"
          onClick={onSkip}
          className="h-11 px-2 text-sm text-zinc-400 transition hover:text-zinc-600 dark:hover:text-zinc-300"
        >
          فعلاً رد کردن
        </button>
      </div>
